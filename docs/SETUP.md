...
Для локальной сборки фронтенда нужен `Node.js 20+`.
...
4. Подключить `public/storage`:

```bash
./vendor/bin/sail artisan storage:link
```

5. Применить миграции и заполнить каталог:
...
6. Если используется `SEARCH_DRIVER=meilisearch`, нужно подготовить индекс:
...
7. Сгенерировать OpenAPI:

```bash
./vendor/bin/sail composer docs:openapi
```

После этого спецификация будет доступна по адресам:

- `http://localhost/openapi.yaml`
- `http://localhost/openapi.json`

8. Собрать фронтенд-ассеты:
...
Посмотреть доступные команды проекта:
...
Проверить OpenAPI-маршрут:

```bash
./vendor/bin/sail artisan route:list --path=openapi
```
